...
(function() {
  ...
  configure = function(options) {
    ...
    if (options.template) {
      ...
      config.js = options.js || path.join(dire, 'jump_menu.js');
      config.template = _.template(fs.readFileSync(config.template).toString());
      config.sources = options.args.filter(function(source) {}, lang = getLanguage(source, config), !lang(lang) ? console.warn("docco: skipped unknown type (" + (path.basename(source)) + ")") : void 0).sort();
    }
    return config;
  };
